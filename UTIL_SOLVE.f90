...
subroutine ADSolver()
    ...
    DO iter=1,AD_itermax !WHILE (error>errormax)
        !IF (solvetype_AD .EQ. 2) THEN
            DO j=2,ny-1
                ...
                DO i=1,nx-2
                    i2d = i+1
                    un(i) = u(i2d,j) - (dt/(2*dx))*(u(i2d+1,j)*uf(i2d+1,j-1) - u(i2d-1,j)*uf(i2d-1,j-1)) &
                ...
                DO i=1,nx-2
                    i2d = i+1
                    vn(i) = v(i2d,j) - (dt/(2*dx))*(v(i2d+1,j)*uf(i2d+1,j-1) - v(i2d-1,j)*uf(i2d-1,j-1)) &
        ...
        !     DO j=2,ny-1
        !         DO i=2,nx-1
        !             u(i,j) = u(i,j) - (dt/(2*dx))*(u(i+1,j)*uf(i+1,j-1) - u(i-1,j)*uf(i-1,j-1)) &
        !                             - (dt/(2*dy))*(u(i,j+1)*vf(i-1,j+1) - u(i,j-1)*vf(i-1,j-1)) &
        !                             + (dt/(Re*(dx**2)))*(u(i+1,j) + u(i-1,j)) &
        !                             + (dt/(Re*(dy**2)))*(u(i,j+1) + u(i,j-1)) 
        !             u(i,j) = u(i,j)/(1 + ((2*dt)/(Re*(dx**2))) + ((2*dt)/(Re*(dx**2))))      
                    
        !             v(i,j) = v(i,j) - (dt/(2*dx))*(v(i+1,j)*uf(i+1,j-1) - v(i-1,j)*uf(i-1,j-1)) &
        !                             - (dt/(2*dy))*(v(i,j+1)*vf(i-1,j+1) - v(i,j-1)*vf(i-1,j-1)) &
        !                             + (dt/(Re*(dx**2)))*(v(i+1,j) + v(i-1,j)) &
        !                             + (dt/(Re*(dy**2)))*(v(i,j+1) + v(i,j-1)) 
        !             v(i,j) = v(i,j)/(1 + ((2*dt)/(Re*(dx**2))) + ((2*dt)/(Re*(dx**2))))
        !         END DO
        !     END DO
        !     errorx = MAXVAL(ABS(ukp1) - ABS(uk))
        !     uk(:,:) = ukp1(:,:)
